...
namespace GMap.NET.CacheProviders
{
   ...
   public class SQLitePureImageCache : PureImageCache
   {
      ...
      static readonly string singleSqlInsertForCache = "INSERT INTO main.Tiles(X, Y, Zoom, Type, CacheTime, gid, parent_gid) VALUES(@p1, @p2, @p3, @p4, @p5, @p6, @p7)";
                     ...
                     using(DbTransaction tr = cn.BeginTransaction())
                     {
                        try
                        {
                           using(DbCommand cmd = cn.CreateCommand())
                           {
                              ...
                              // 带上 gid 和 parent_gid
                              cmd.CommandText = singleSqlInsertForCache;
                              ...
                              cmd.Parameters.Add(new SQLiteParameter("@p6", pos.gid));
                              cmd.Parameters.Add(new SQLiteParameter("@p7", pos.parentGid));
