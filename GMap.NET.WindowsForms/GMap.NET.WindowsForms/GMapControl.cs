   ...
   using OpenNETCF.ComponentModel;
    ...
    public partial class GMapControl : UserControl, Interface
    {
      ...
      internal readonly Font MissingDataFont = new Font(FontFamily.GenericSansSerif, 8, FontStyle.Regular);
        ...
        public GPoint RenderOffset { get { return Core.renderOffset; } }
        /// <summary>
        /// 修改原有UpdateRouteLocalPosition方法，新增一个参数，用于线路判断在zoom缩放等级是否有graphicsPath; by gujian 20170908
        /// </summary>
        /// <param name="route"></param>
        public void UpdateRouteLocalPosition(GMapRoute route, int zoom1)
        {
            if (route.RouteGraphicsPath[zoom1].PointCount > 0)
            {
                return;
            }
            else
...
#if !PocketPC
                route.UpdateGraphicsPath(zoom1);
        ...
        public void ZoomAndCenterMarker(GMapMarker m)
        {
            double left = m.Position.Lng;
            double top = m.Position.Lat;
            double right = m.Position.Lng;
            double bottom = m.Position.Lat;
            this.Position = new PointLatLng(top, left);
            this.Zoom = 15f;
        }
        /// <summary>
      ...
      protected override void OnPaintBackground(PaintEventArgs e)
      {
        ...
        void DrawGraphics(Graphics g)
        {
...
#endif
            {
...
#endif
                {
                    ...
                    ////  Debug.WriteLine("地图底图耗时：" + delta.ToString());

                    //  DateTime start1 = DateTime.Now;
                    OnPaintOverlays(g);
                    //DateTime end1 = DateTime.Now;
                    //int delta1 = (int)(end1 - start1).TotalMilliseconds;
                    //Debug.WriteLine("平移地图后或默认绘制图层耗时：" + delta1.ToString());
                    ...
                    if (!isDragging)
                    {
                        ...
                        //  int delta2 = (int)(end - start).TotalMilliseconds;
                        //  Debug.WriteLine("平移地图后或默认求坐标耗时：" + delta2.ToString());
        ...
        protected virtual void OnPaintOverlays(Graphics g)
        {
            ...
            for (int i = 0; i < Overlays.Count; i++)
            {
                ...
                //    return;
                //}
                if (Overlays[i].IsVisibile)
                {
                    Overlays[i].OnRender(g);
                    if (!isMouseDown)
                    {
                        Overlays[i].LableOnRender(g);
                    }
                }
            }
            ...
            else
            {
                    ...
                    if (Core.mouseDown.IsEmpty)
                {
                    for (int i = Overlays.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (o != null && o.IsVisibile)
                        {
                            for (int im = 0; im < o.Markers.Count; im++)
                            {
                                if (o.Markers[im] == null)
                                {
                                    return;
                                }
                                if (o.Markers[im].IsVisible && o.Markers[im].IsHitTestVisible)
                                {
                                    #region -- check --
...
#endif
                                    if (o.Markers[im].LocalArea.Contains((int)rp.X, (int)rp.Y))
                                    {
                                        if (!o.Markers[im].IsMouseOver)
                                        {
#if !PocketPC
                                            SetCursorHandOnEnter();
#endif
                                            o.Markers[im].IsMouseOver = true;
                                            IsMouseOverMarker = true;

                                            if (OnMarkerEnter != null)
                                            {
                                                OnMarkerEnter(o.Markers[im]);
                                            }
                                    ...
                                    else if (o.Markers[im].IsMouseOver)
                                    {
                                        o.Markers[im].IsMouseOver = false;
                            ...
                            //   for (int index = 0; index < o.Routes.Count; index++)
                            //  {
                            //   if (o.Routes[index].IsVisible && o.Routes[index].IsHitTestVisible)
                            //  {
                            #region -- check --
                            ...
                            //   }
#endif
                            // if (o.Routes[index].IsInside((int)rp.X, (int)rp.Y))
                            // {
                            // if (!o.Routes[index].IsMouseOver)
                            // {
#if !PocketPC
                            //   SetCursorHandOnEnter();
#endif
                            //   o.Routes[index].IsMouseOver = true;
                            //   IsMouseOverRoute = true;

                            //  if (OnRouteEnter != null)
                            //   {
                            //    OnRouteEnter(o.Routes[index]);
                            // }
                            ...
                            // if (o.Routes[index].IsMouseOver)
                            // {
                            //  o.Routes[index].IsMouseOver = false;
                ...
                //DateTime end = DateTime.Now;
                //int delta = (int)(end - start).TotalMilliseconds;
                //Debug.WriteLine("求是否在要素上耗时：" + delta.ToString());
#if !PocketPC
                if (renderHelperLine)
                {
                    base.Invalidate();
